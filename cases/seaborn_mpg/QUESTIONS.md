...
- According to cell [3], what years and origins does this dataset cover?
...
- What category of missingness would you assume (systematic vs. random), and in what sense does it matter for the drop-vs-impute decision?
...
**Q5** · `eda.confounding` · `eda.data-sampling`

- Cell [6]: Which combination of origin and cylinder count is likely over-represented in the data?
- Cell [6] shows median mpg of 31.6 (japan), 26.0 (europe), and 18.5 (usa). The docstring says origin is "partly a proxy for cylinders/displacement." What would happen to the estimated coefficient of `origin` if `cylinders` and `displacement` were also in the model?
...
- Cell [8] ranks weight (-0.832), displacement (-0.805), horsepower and cylinders (both -0.778) as the strongest correlators with mpg. Cell [9] shows these same four features are intercorrelated at r > 0.9. Would you expect all four to add independent predictive power, or mostly redundant information?

**Q8** · `eda.general`

- In cell [9]: Why does high multicollinearity of features inflate OLS (ordinary least squares) coefficient variance?
...
- Cell [2] `pd.get_dummies(..., drop_first=True)` keeps `origin_japan` and `origin_usa` but drops `origin_europe`. What does that imply for reading the two dummy coefficients?
...
- In cell [4], `StandardScaler` is fitted on `X_train` and `y_train` only. What could go wrong if they were fitted on the full dataset before splitting?
...
- Given the multicollinearity found in the EDA script (high correlation among 4 features), why is Ridge a reasonable starting choice over plain OLS?

**Q6** · `modeling.param-effect`

- `ALPHA_MAX = 0.1` defined in cell [0] caps the alpha search range, which is fed into both `ALPHAS_COARSE` and `ALPHAS_FINE` in cell [6]. Would it make sense to extend `ALPHA_MAX` to 1.0 to consider the full range of trade-offs between loss functions and regularization?
...
- As in cell [5], observe that validation errors in cell [8] are consistently *below* train errors. Why can that be and what does that mean?
...
- Cell [10]: The output reports "Lasso zeroed 4/8 features at alpha = 0.014." Given the high correlations among engine-size features found in `mpg_01_eda.py`, which 4 features would you predict are zeroed? (take a look at the coefficient paths from cell [9] to attempt  confirmation)
- Lasso test RMSE (3.260 mpg) is nearly identical to OLS (3.256 mpg) despite using only 4 active features. What does this tell you about the relative information content of the zeroed features?
...
- Cell [3] compares learning curves for α=1000 (extremely high regularization -> high model bias) vs. α=0.01 (low regularisation). Both end with a gap of ~0.6-0.7 mpg at the largest training size.
...
- Is there a mathematical argument that training error is always rising with regularization?
...
- Would actually pick the tree model with max_depth=8 or prefer another (simpler) one?
...
- Cell [5]: Here you see the the *identical* stability experiment (30 seeds, single 80/20 split vs. 5-fold CV) on the same data. What's the one thing that differs between the two cell [5]s from `mpg_03` and `mpg_04`, and why does that make the comparison across scripts fair?
